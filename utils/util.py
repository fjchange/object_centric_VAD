...
def split_path_boxes(path_box_list,dataset_name,img_height,img_width):
    ...
    for item in path_box_list:
        ...
        for i in range(1,len(path_list)-1):
            temp_path=osp.join(temp_path,path_list)
        ...
        boxes.append([int(item[1][0]*img_height),int(item[1][1]*img_width),
                      int(img_height*(item[0][2]-item[0][0])),int(img_width*(item[0][3]-item[0][1]))])
...
def _crop_img(path,box,target_size):
    ...
    jpg=tf.image.resize_bilinear(jpg,size=(target_size,target_size))
...
def CAE_dataset(np_path_box,dataset_name,num_epochs,batch_size):
    ...
    (image_width, image_height) = Image.open(path_box_list[0]).size
    ...
    input_queue=tf.train.slice_input_producer([former_paths,gray_paths,back_paths,boxes],num_epochs=num_epochs,shuffle=True)
    ...
    grad1=tf.image.image_gradients(crop_img_1)
    grad2=tf.image.image_gradients(crop_img_2)
    grad3=tf.image.image_gradients(crop_img_3)

    grad_dis_1=grad2-grad1
    grad_dis_2=grad3-grad2

    gray_batch,former_batch,back_batch=tf.train.batch([crop_img_2,grad_dis_1,grad_dis_2],batch_size=batch_size,capacity=1000)
